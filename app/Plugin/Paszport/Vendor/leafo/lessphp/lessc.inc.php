...
class lessc
{
    ...
    {
        $locale = setlocale(LC_PASZPORT_NUMERIC, 0);
        setlocale(LC_PASZPORT_NUMERIC, "C");
        ...
        $out = ob_get_clean();
        setlocale(LC_PASZPORT_NUMERIC, $locale);
...
rotected function tag(&$tag, $simple = false)
{
	...
	p
rotected function tagExpression(&$value)
{
    $s = $this->seek();
	...
	p
rotected function tagBracket(&$value)
{
    // speed shortcut
    if (isset($this->buffer[$this->count]) && $this->buffer[$this->count] != "[") {
        return false;
...
rotected function to($what, &$out, $until = false, $allowNewline = false)
{
	...
	p
rotected function argumentDef(&$args, &$isVararg, $delim = ',')
{
    $s = $this->seek();
    if (!$this->literal('(')) {
        return false;
...
rotected function mixinTags(&$tags)
{
	...
	p
rotected function argumentValues(&$args, $delim = ',')
{
    $s = $this->seek();
    if (!$this->literal('(')) {
        return false;
