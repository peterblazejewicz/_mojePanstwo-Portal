(function () {
    ...
    function e(a) {
        var b = a.length, c = 0;
        while (++c < b)if (a[c - 1] >= a[c])return !1;
        return !0
    }

    function d(a) {
        var b = a.length, c = -1;
        while (++c < b)if (!isFinite(a[c]))return !1;
        return !0
    }

    function c(a, b, c, d) {
        var e = [], f = a + c, g = b.length, h = -1;
    ...
    function b(b, c) {
        var d = c.length;
        if (b > d)return null;
        var e = [], f = [], g = {}, h = 0, i = 0, j, k, l;
        while (i < b) {
            if (h === d)return null;
            var m = Math.floor(Math.random() * d);
            if (m in g)continue;
            g[m] = 1, h++, k = c[m], l = !0;
            for (j = 0; j < i; j++)if (a(k, e[j])) {
                l = !1;
                break
            }
            l && (e[i] = k, f[i] = m, i++)
    ...
    science.stats = {}, science.stats.bandwidth = {
        nrd0: function (a) {
            ...
            (lo = Math.min(b, science.stats.iqr(a) / 1.34)) || (lo = b) || (lo = Math.abs(a[1])) || (lo = 1);
            return .9 * lo * Math.pow(a.length, -0.2)
        }, nrd: function (a) {
        ...
        euclidean: function (a, b) {
            ...
            var c = a.length, d = -1, e = 0, f = 0, g, h;
            while (++d < c)g = a[d], h = b[d], e += Math.abs(g - h), f += Math.abs(g + h);
            return e / f
        }
    }, science.stats.erf = function (a) {
        var b = .254829592, c = -0.284496736, d = 1.421413741, e = -1.453152027, f = 1.061405429, g = .3275911, h = a < 0 ? -1 : 1;
        a < 0 && (h = -1, a = -a);
        var i = 1 / (1 + g * a);
        ...
        function d(d, e) {
            ...
            return d.map(function (c) {
                var d = -1, e = 0, g = b.length;
                while (++d < g)e += a((c - b[d]) / f);
                return [c, e / f / g]
            })
        }

        var a = science.stats.kernel.gaussian, b = [], c = science.stats.bandwidth.nrd;
        d.kernel = function (b) {
        ...
        function f(f) {
            var g = f.length, h = [], i = [], j = 1, l = 0, m = b(e, f), n, o, p, q, r, s, t;
            while (j && l < d) {
                p = -1;
                while (++p < e)i[p] = 0;
                o = -1;
                while (++o < g) {
                    ...
                    while (++p < e)r = c.call(this, m[p], q), r < s && (s = r, t = p);
                    i[h[o] = t]++
                }
                n = [], o = -1;
                while (++o < g) {
                    q = h[o], r = n[q];
                    if (r == null)n[q] = f[o].slice(); else {
                        p = -1;
                ...
                j = 0, p = -1;
                while (++p < e)if (!a(n[p], m[p])) {
                    j = 1;
                    break
                }
                m = n, l++
            }
            return {assignments: h, centroids: m}
        }

        var c = science.stats.distance.euclidean, d = 1e3, e = 1;
        f.k = function (a) {
        ...
        function d(d) {
            var e = d.length, f = [], g = [], h = [], i = [], j, k, l, m, n, o, p, q;
            p = -1;
            while (++p < e) {
                f[p] = 0, h[p] = [], q = -1;
                while (++q < e)h[p][q] = p === q ? Infinity : a(d[p], d[q]), h[p][f[p]] > h[p][q] && (f[p] = q)
            }
            p = -1;
            while (++p < e)i[p] = [], i[p][0] = {
                left: null,
            ...
            for (n = 0; n < e - 1; n++) {
                ...
                k = f[j], l = i[j][0], m = i[k][0], newCluster = {
                    left: l,
                    right: m,
                    dist: h[j][k],
                    centroid: c(l.size, l.centroid, m.size, m.centroid),
                    size: l.size + m.size,
                    depth: 1 + Math.max(l.depth, m.depth)
                }, i[j].splice(0, 0, newCluster), g[j] += g[k];
        ...
        var a = science.stats.distance.euclidean, b = "simple";
        ...
        function h(h, i, j) {
            var k = h.length, l;
            if (k !== i.length)throw{error: "Mismatched array lengths"};
            if (k == 0)throw{error: "At least one point required."};
            ...
            if (m < 2)throw{error: "Bandwidth too small."};
            var n = [], o = [], p = [];
            l = -1;
            ...
            while (++q <= b) {
                ...
                while (++l < k) {
                    s = h[l], l > 0 && g(h, j, l, r);
                    var t = r[0], u = r[1], v = h[l] - h[t] > h[u] - h[l] ? t : u, w = 0, x = 0, y = 0, z = 0, A = 0, B = Math.abs(1 / (h[v] - s));
                    for (var C = t; C <= u; ++C) {
                        var D = h[C], E = i[C], F = C < l ? s - D : D - s, G = f(F * B) * p[C] * j[C], H = D * G;
                        w += G, x += H, y += D * H, z += E * G, A += E * H
                    }
                    var I = x / w, J = z / w, K = A / w, L = y / w, M = Math.sqrt(Math.abs(L - I * I)) < c ? 0 : (K - I * J) / (L - I * I), N = J - M * I;
        ...
        h.bandwidth = function (b) {
            ...
            a = b;
            return h
        }, h.robustnessIterations = function (a) {
            if (!arguments.length)return a;
            b = a;
            return h
        }, h.accuracy = function (a) {
            if (!arguments.length)return a;
            c = a;
            return h
        };
        ...
        var b, c = a.length, d = -1, e = d, f = null, g = 0, h, i;
        while (++d < c)(i = a[d]) !== f && ((h = d - e) > g && (g = h, b = f), f = i, e = d);
        return b
        ...
        return b.map(function (b) {
            if (b === 0)return a[0];
            if (b === 1)return a[c];
            var d = 1 + b * c, e = Math.floor(d), f = d - e, g = a[e - 1];
        ...
        if (b < 1)return NaN;
        if (b === 1)return 0;
        var c = science.stats.mean(a), d = -1, e = 0;
        while (++d < b) {
            var f = a[d] - c;
            e += f * f
        }
