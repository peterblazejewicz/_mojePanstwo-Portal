    ...
    "path": {
        ...
        "with an equirectangular projection": {
            ...
            "observes the correct winding order of a tiny polygon": function (p) {
                ...
                assert.equal(testContext.buffer().filter(function (d) {
                    return d.type === "moveTo";
                }).length, 1);
            },
        ...
        "with the default context (null) and default projection (albers-usa)": {
            ...
            "centroid of a line string": function (p) {
                assert.inDelta(p.centroid({
                    type: "LineString", coordinates: [
                        [-122, 37],
                        [-74, 40],
                        [-100, 0]
                    ]
                }), [545.131, 253.860], 1e-3);
            }
        },
        ...
        "with an equirectangular projection rotated by [180, -248]": {
            ...
            "renders a polygon": function (p) {
                p({
                    type: "Polygon", coordinates: [
                        [
                            [-175.03150315031502, 66.57410661866186],
                            [-174.34743474347434, 66.33097912391239],
                            [-174.5994599459946, 67.0603616081608],
                            [-171.86318631863185, 66.90406536153614],
                            [-169.9189918991899, 65.96628788178816],
                            [-170.89108910891088, 65.53213164116411],
                            [-172.54725472547256, 65.42793414341432],
                            [-172.5832583258326, 64.45542416441643],
                            [-172.97929792979298, 64.2470291689169],
                            [-173.91539153915392, 64.28176166816681],
                            [-174.67146714671466, 64.62908666066605],
                            [-176.003600360036, 64.90694665466546],
                            [-176.21962196219621, 65.34110289528951],
                            [-177.22772277227722, 65.51476539153916],
                            [-178.37983798379838, 65.37583539453945],
                            [-178.91989198919893, 65.72316038703869],
                            [-178.7038703870387, 66.10521787878787],
                            [-179.8919891989199, 65.8620903840384],
                            [-179.45994599459945, 65.3932016441644],
                            [-180, 64.97641165316531],
                            [-180, 68.95328281728172],
                            [-177.55175517551754, 68.18916783378336],
                            [-174.95949594959495, 67.19929160516051],
                            [-175.03150315031502, 66.57410661866186]
                        ]
                    ]
                });
                assert.deepEqual(testContext.buffer().filter(function (d) {
                    return d.type === "moveTo";
                }), [
        ...
        "pointRadius": {
            ...
            "coerces point radius to a number": {
                "when the radius is specified as a constant": function (path) {
                    ...
                    assert.strictEqual(testContext.buffer().filter(function (d) {
                        return d.type === "arc";
                    })[0].r, 6);
                },
                "when the radius is specified as a function": function (path) {
                    var p = path().projection(null).context(testContext).pointRadius(function () {
                        return "6";
                    });
                    p({type: "Point", coordinates: [0, 0]});
                    assert.strictEqual(testContext.buffer().filter(function (d) {
        ...
        "with an equirectangular projection clipped to 90° and rotated by [-17°, -451°]": {
            "renders a polygon": function (path) {
                var pole = _.range(-180, 180, 10).map(function (x) {
                    return [x, 70];
                });
                pole.push(pole[0]);
                path()
        ...
        "with an equirectangular projection rotated by [34.5°, 90°]": {
            ...
            "observes proper clip point ordering for lines": function (p) {
                var line = _.range(-90, 180, 10).map(function (x) {
                    return [x, 20];
                })
                    .concat(_.range(170, -100, -10).map(function (x) {
                        return [x, 0];
                    }))
                    .concat([
                        [-90, 20]
        ...
        "with an Albers projection and adaptive resampling": {
            "correctly resamples near the poles": function (path) {
                ...
                assert.isTrue(testContext.buffer().filter(function (d) {
                    return d.type === "lineTo";
                }).length > 1);
                p({
                    type: "LineString", coordinates: [
                        [180, 90],
...
var testContext = {
    arc: function (x, y, r, a0, a1) {
        testBuffer.push({type: "arc", x: Math.round(x), y: Math.round(y), r: r});
    },
    moveTo: function (x, y) {
        testBuffer.push({type: "moveTo", x: Math.round(x), y: Math.round(y)});
    },
    lineTo: function (x, y) {
        testBuffer.push({type: "lineTo", x: Math.round(x), y: Math.round(y)});
    },
    closePath: function () {
        testBuffer.push({type: "closePath"});
    },
    buffer: function () {
        var result = testBuffer;
        testBuffer = [];
        return result;
    }
};
...
function stripes(a, b) {
    return {
        type: "Polygon", coordinates: [a, b].map(function (d, i) {
            var stripe = _.range(-180, 180, 1).map(function (x) {
                return [x, d];
            });
