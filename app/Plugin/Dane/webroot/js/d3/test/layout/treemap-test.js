...
suite.addBatch({
    "treemap": {
        ...
        "can specify padding as a function that returns a number": function (treemap) {
            var t = treemap().size([1000, 1000]).sort(null).padding(function (d) {
                return d.depth;
            });
            assert.deepEqual(t.nodes({
                children: [
        ...
        "no negatively sized rectangles": function (treemap) {
            var t = treemap().size([615, 500]).sort(function (a, b) {
            ...
            assert.equal(nodes.filter(function (n) {
                return n.dx < 0 || n.dy < 0;
            }).length, 0);
        },
        "no overhanging rectangles": function (treemap) {
            var t = treemap().size([100, 100]).sort(function (a, b) {
                    return a.value - b.value;
                }),
            ...
            assert.equal(nodes.filter(function (n) {
                return n.dx < 0 || n.dy < 0 || n.x + n.dx > 100 || n.y + n.dy > 100;
            }).length, 0);
        },
        "can handle an empty children array": function (treemap) {
            ...
            assert.deepEqual(treemap().nodes({
                children: [
                    {children: []},
                    {value: 1}
                ]
        ...
        "slice-dice": function (treemap) {
            assert.deepEqual(treemap().size([100, 10]).mode("slice-dice").nodes({
                children: [
                    {
                        children: [
                            {value: 1},
