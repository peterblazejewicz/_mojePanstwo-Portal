...
<? if ($dzialy = $object->getLayer('pkd_dzialy')) { ?>
    <ul class="chart-blocks">
        <? foreach ($dzialy as $dzial) { ?>
            ...
            <li style="background-color: #<?= $dzial['color'] ?>; width: <?= $dzial['score'] * 100 ?>%">
                <a href="#" title="<?= addslashes($dzial['nazwa']) ?>"><?= $dzial['nazwa'] ?></a>
            </li>
    ...
    <div class="profile_baner" data-adres="<?= urlencode($adres) ?>">
        <div class="bg">
            ...
            <div class="content">
                ...
                <button
                    class="googleMap btn btn-info"><?= __d('dane', 'LC_DANE_VIEW_KRSPODMIOTY_OTWORZ_MAPE') ?></button>
                <button
                    class="streetView btn btn-info"><?= __d('dane', 'LC_DANE_VIEW_KRSPODMIOTY_OTWORZ_MAPE_STREET') ?></button>
            </div>
        </div>
        <div class="googleView">
            ...
            <button class="closeMap glyphicon glyphicon-remove"></button>
