<div class="objectRender sejm_debata_wystapienie<? if ( $alertsStatus ) {echo " unreaded";} else {echo " readed";} ?>" oid="<?php echo $object->getId() ?>" gid="<?php echo $object->getGlobalId() ?>">
	...
	<div class="row<? if( in_array($stanowisko_id, array(3, 4, 130)) ) {?> porzadek<?} else {?> wystapienie<?}?>">
		...
		<div class="sw_avatar">
			...
			<p class="mowca"><? if($mowca_href) {?><a href="<?=$mowca_href?>"><?}?><?= $object->getData('ludzie.nazwa') ?><? if($mowca_href){?></a><?}?> <span class="label label-<?= $label_class ?>"><?= $object->getData('stanowiska.nazwa') ?></span></p>
