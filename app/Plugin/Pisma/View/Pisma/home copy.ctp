    ...
    <? } else { ?>
        ...
        <ul>
            <? foreach ($pisma as $pismo) { ?>
                <li><a href="<?= Router::url(array(
                        'action' => 'edit',
                        'id' => $pismo['id']
                    )) ?>"><?= $pismo['to_dataset'] . ': ' . $pismo['subject'] ?></a>
					<span style="margin-left: 30px; font-size: 70%">(<a
                            href="<?= Router::url(array('action' => 'delete', 'id' => $pismo['id'])) ?>">skasuj</a>)</span>
                </li>
            <? } ?>
        </ul>
