...
<form class="form-horizontal" role="form" method="post">
	...
	<div class="form-group <? if ( isset( $verr['template_id'] ) ) {
		echo 'has-error';
	} ?>">
	...
	<div class="form-group <? if ( isset( $verr['to_dataset'] ) || isset( $verr['to_id'] ) ) {
		echo 'has-error';
	} ?>">
		<label class="col-sm-2 control-label">Odbiorca</label>

		<div class="col-sm-10">
			<input type="text" class="form-control" id="fToType" name="to_dataset" placeholder="Typ odbiorcy"
			       value="<?= $doc['to_dataset'] ?>">
			<input type="number" class="form-control" id="fToId" name="to_id" placeholder="ID odbiorcy"
			       value="<?= $doc['to_id'] ?>">
		</div>
	</div>

	<div class="form-group <? if ( isset( $verr['subject'] ) ) {
		echo 'has-error';
	} ?>">
		<label for="fTitle" class="col-sm-2 control-label">Tytuł</label>

		<div class="col-sm-10">
			<input type="text" class="form-control" id="fTitle" name="subject" placeholder="Wpisz tytuł pisma"
			       value="<?= $doc['subject'] ?>">
		</div>
	</div>

	<div class="form-group <? if ( isset( $verr['content'] ) ) {
		echo 'has-error';
	} ?>">
	...
	<div class="form-group <? if ( isset( $verr['from_name'] ) || isset( $verr['from_email'] ) ) {
		echo 'has-error';
	} ?>">
		...
		<div class="col-sm-10">
			...
			<input type="email" class="form-control" id="fFromEmail" name="from_email"
			       value="<?= $doc['from_email'] ?>">
		</div>
	</div>
