...
class TreeBehavior extends ModelBehavior {
	...
	public function afterDelete( Model $Model ) {
		...
		if ( $diff > 2 ) {
			...
			$scope[][ $Model->escapeField( $left ) . " BETWEEN ? AND ?" ] = array(
				$data[ $left ] + 1,
				$data[ $right ] - 1
			);
		...
		if ( $mode === 'parent' ) {
			...
			$missingParents = $Model->find( 'list', array(
				...
				'conditions' => array(
					$scope,
					array(
						'NOT'                               => array( $Model->escapeField( $parent ) => null ),
						$Model->VerifyParent->escapeField() => null
					)
				)
			) );
	...
	public function verify( Model $Model ) {
		...
		for ( $i = $min; $i <= $edge; $i ++ ) {
			$count = $Model->find( 'count', array(
				'conditions' => array(
					$scope,
					'OR' => array( $Model->escapeField( $left ) => $i, $Model->escapeField( $right ) => $i )
				)
			) );
			if ( $count != 1 ) {
				if ( ! $count ) {
		...
		$node = $Model->find( 'first', array(
			'conditions' => array(
				$scope,
				$Model->escapeField( $right ) . '< ' . $Model->escapeField( $left )
			),
			'recursive'  => 0
		) );
		if ( $node ) {
		...
		foreach ( $Model->find( 'all', array( 'conditions' => $scope, 'recursive' => 0 ) ) as $instance ) {
			if ( $instance[ $Model->alias ][ $left ] === null || $instance[ $Model->alias ][ $right ] === null ) {
				$errors[] = array(
					'node',
					$instance[ $Model->alias ][ $Model->primaryKey ],
				...
				'conditions' => array(
					$scope,
					$Model->escapeField( $left ) . ' <'  => $instance[ $Model->alias ][ $left ],
					$Model->escapeField( $right ) . ' >' => $instance[ $Model->alias ][ $right ]
				),
				'recursive'  => 0
			) )
			) {
				$errors[] = array(
					'node',
					$instance[ $Model->alias ][ $Model->primaryKey ],
					'The parent field is blank, but has a parent'
				);
			}
		}
		if ( $errors ) {
