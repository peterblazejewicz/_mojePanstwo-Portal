...
class FixtureTask extends BakeTask {
	...
	public function importOptions( $modelName ) {
		...
		if ( ! empty( $this->params['schema'] ) ) {
			...
			$doSchema = $this->in( __d( 'cake_console', 'Would you like to import schema for this fixture?' ), array(
				'y',
				'n'
			), 'n' );
			if ( $doSchema === 'y' ) {
		...
		if ( ! empty( $this->params['records'] ) ) {
			...
			$doRecords = $this->in( __d( 'cake_console', 'Would you like to use record importing for this fixture?' ), array(
				'y',
				'n'
			), 'n' );
		}
