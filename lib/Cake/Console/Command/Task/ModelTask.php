...
class ModelTask extends BakeTask {
	...
	public function getTable( $modelName, $useDbConfig = null ) {
		...
		if ( $this->interactive === true ) {
			...
			if ( array_search( $useTable, $this->_tables ) === false ) {
				...
				$tableIsGood = $this->in( __d( 'cake_console', 'Do you want to use this table?' ), array(
					'y',
					'n'
				), 'y' );
			}
	...
	public function fieldValidation( $fieldName, $metaData, $primaryKey = 'id' ) {
		...
		while ( $anotherValidator === 'y' ) {
			...
			$guess   = $defaultChoice;
			if ( $metaData['null'] != 1 && ! in_array( $fieldName, array(
					$primaryKey,
					'created',
					'modified',
					'updated'
			...
			if ( $this->interactive && $choice != $defaultChoice ) {
				$anotherValidator = $this->in( __d( 'cake_console', 'Would you like to add another validation rule?' ), array(
					'y',
					'n'
				), 'n' );
			}
	...
	public function doMoreAssociations( Model $model, $associations ) {
		...
		while ( strtolower( $wannaDoMoreAssoc ) === 'y' ) {
			...
			$wannaDoMoreAssoc = $this->in( __d( 'cake_console', 'Define another association?' ), array(
				'y',
				'n'
			), 'y' );
		}
