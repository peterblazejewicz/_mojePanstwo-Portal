...
class ExtractTask extends AppShell {
	...
	public function execute() {
		...
		if ( isset( $this->params['extract-core'] ) ) {
			...
			$response           = $this->in( __d( 'cake_console', 'Would you like to extract the messages from the CakePHP core?' ), array(
				'y',
				'n'
			), 'n' );
		...
		if ( isset( $this->params['merge'] ) ) {
			...
			$this->out();
			$response     = $this->in( __d( 'cake_console', 'Would you like to merge all domain and category strings into the default.pot file?' ), array(
				'y',
				'n'
			), 'n' );
