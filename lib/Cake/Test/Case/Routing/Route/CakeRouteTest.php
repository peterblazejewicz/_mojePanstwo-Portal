...
class CakeRouteTest extends CakeTestCase {
	...
	public function testBasicRouteCompiling() {
		...
		$route  = new CakeRoute( '/:plugin/:controller/:action/*', array(
			'plugin' => 'test_plugin',
			'action' => 'index'
		) );
		$result = $route->compile();
	...
	public function testParseWithHttpHeaderConditions() {
		...
		$route                     = new CakeRoute( '/sample', array(
			'controller' => 'posts',
			'action'     => 'index',
			'[method]'   => 'POST'
		) );
	...
	public function testMatchGreedyNamedExcludesPrefixes() {
		...
		$this->assertFalse( $route->match( array(
			'controller' => 'sales',
			'action'     => 'index',
			'admin'      => 1
		) ), 'Greedy named consume routing prefixes.' );
	}
