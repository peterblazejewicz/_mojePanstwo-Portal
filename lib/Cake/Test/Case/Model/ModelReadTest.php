...
class ModelReadTest extends BaseModelTest {
	...
	public function testVeryStrangeUseCase() {
		...
		$param = array(
			...
			$this->db->fullTableName( 'articles' ) . '.user_id',
			'3',
			$this->db->fullTableName( 'articles' ) . '.published',
			'Y'
		);
	...
	public function testBuildQuery() {
		...
		$result   = $TestModel->buildQuery( 'all', array(
			'returnQuery' => true,
			'conditions'  => array( 'user' => 'larry' )
		) );
		$this->assertEquals( $expected, $result );
	}
	...
	public function testFindAll() {
		...
		$result = $TestModel->find( 'all', array(
			'conditions' => array(
				'or' => array( 'User.id' => '0', 'User.user LIKE' => '%a%' )
			)
		) );
